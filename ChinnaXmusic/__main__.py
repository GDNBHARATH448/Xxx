...
import config
from ChinnaXmusic import LOGGER, app, userbot
from ChinnaXmusic.core.call import Aviax
from ChinnaXmusic.misc import sudo
from ChinnaXmusic.plugins import ALL_MODULES
from ChinnaXmusic.utils.database import get_banned_users, get_gbanned
...
async def init():
    ...
    for all_module in ALL_MODULES:
        importlib.import_module("ChinnaXmusic.plugins" + all_module)
    LOGGER("ChinnaXmusic.plugins").info("Successfully Imported Modules...")
    await userbot.start()
    await Aviax.start()
    try:
        await Aviax.stream_call("https://te.legra.ph/file/29f784eb49d230ab62e9e.mp4")
    except NoActiveGroupCall:
        LOGGER("ChinnaXmusic").error(
    ...
    await app.stop()
    await userbot.stop()
    LOGGER("ChinnaXmusic").info("Stopping Aviax Music Bot...")
